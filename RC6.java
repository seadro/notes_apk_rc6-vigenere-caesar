...
public class RC6 {
    ...
    private static int[] convBytesWords(byte[] key, int u, int c) {
        ...
        for (int i = 0, off = 0; i < c; i++)
            ...
            Log.e("RC6", "Error parsing date retrieved from database!");
        }
        return tmp;
    ...
    private static byte[] paddingKey(byte[] key)  {
     ...
     private     String rightPadZeros(String str, int num)   {
    ...
    public   String encrypt(String plaintext, String cryptoKey)    {
        ...
        byte[] key=rightPadZeros(cryptoKey,16).getBytes();
        ...
        String s = Base64.encodeToString(tmp,Base64.NO_WRAP);
